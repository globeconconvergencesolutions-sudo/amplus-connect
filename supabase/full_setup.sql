-- Amplus Connect — consolidated database setup
-- Combines, in correct final form, the three migrations in supabase/migrations/:
--   20260906111619_454a86b4-...sql   (schema, RLS, seed data)
--   20260906111631_3efac4d6-...sql   (function EXECUTE lockdown)
--   20260906120000_fix_anon_is_staff_grant.sql (re-grant EXECUTE to anon)
-- Run this ONCE against a fresh Supabase project (SQL Editor, or
-- `supabase db execute -f supabase/full_setup.sql`). It is not itself a
-- migration file — do not drop it into supabase/migrations, since the three
-- files above are already tracked there and this would duplicate them.
...
-- is_staff/has_role are evaluated inside RLS USING clauses for anon queries too
-- (e.g. products/services/projects/posts public-read policies below), and
-- Postgres checks EXECUTE privilege on every function a policy references for
-- the querying role even when the boolean would short circuit at runtime. Both
-- functions are security definer and simply check user_roles; called with
-- auth.uid() = null for an anonymous request they safely return false, so
-- granting EXECUTE to anon does not expose any additional data.
...
-- (same as supabase/migrations/20260910120000_media_gallery_stock.sql).
